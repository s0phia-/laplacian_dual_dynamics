...
class Lstdq:
    ...
    def __init__(self,
        ...
        # setup
        self.len_state_rep = 2
        self.matrix_A = np.zeros([self.len_state_rep * self.num_actions, self.len_state_rep * self.num_actions])
        self.vector_b = np.zeros([self.len_state_rep * self.num_actions])
    ...
    def apply_bf(self, state, action):
        sa_bf = np.zeros([self.num_actions, self.len_state_rep])
        sa_bf[action] = state
    ...
    def fit(self):
        ...
        return policy.reshape([self.num_actions, self.len_state_rep])


class LspiAgentNoEVs:
    def __init__(self,
                 env,
                 actions,
        ...
        self.policy = np.random.rand(self.num_actions, self.len_state_rep)
